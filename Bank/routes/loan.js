...
router.post('/', [
  ...
  var sqlquery_= "select concat(Fname,' ',Mname,' ',Lname) as FullName,bankName,loan.Amount,create_Date from customer cu inner join loan on cu.ID=loan.cust_ID inner join bank on loan.bank_id=bank.id where create_date<'"+
  date+"' order by "+order+";";
...
+"<tr>"+th+"Full Name</th>"+th+"Bank Name</th>"+th+"Amount</th>"+th+"Loan Date</th></tr>";

    for (i = 0; i < results.length; i++) {
      code+="<tr>"+td+results[i].FullName+"</td>"+td+results[i].bankName+"</td>"+td+results[i].Amount+"</td>"+td+results[i].create_Date+"</td></tr>";
    }
        code+="</Table></body>";
