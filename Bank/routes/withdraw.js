...
router.post('/', [
  ...
  const order = req.body.order;
      ...
      var sqlquery_= "select concat(Fname,' ',Mname,' ',Lname) as FullName,bankName,withdraws.Amount,Wit_Date from customer cu inner join account acc on cu.id=acc.Cust inner join withdraws on Acc.ID=withdraws.ACC_ID inner join bank on acc.bank_id=bank.id where wit_date<'"+
      date+"' order by "+order+";";
...
+"<tr>"+th+"Full Name</th>"+th+"Bank Name</th>"+th+"Amount</th>"+th+"Withdrawal Date</th></tr>";

        for (i = 0; i < results.length; i++) {
          code+="<tr>"+td+results[i].FullName+"</td>"+td+results[i].bankName+"</td>"+td+results[i].Amount+"</td>"+td+results[i].Wit_Date+"</td></tr>";
        }
        code+="</Table></body>";
