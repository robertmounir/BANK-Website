...
router.get('/', ensureAuthenticated,(req, res) => {
  ...
  db.query(sqlquery, (err, results, fields) => {
    ...
    if (results[0]) {
      db.query("select * from deposits where Cust_ID = " + req.user.ID , (err, results, fields) => {
        if(err) return console.log(err);
        history = results;
        db.query("select max(Amount) as max, min(Amount) as min, avg(Amount) as avg from deposits where Cust_ID = " + req.user.ID, (err, results2, fields) => {
          if(err) return console.log(err);
          statistics = results2[0];
          return res.render('customerdeposite', {
            title: "customerdeposite",
            css: "customerdeposite",
            href1: "customer",
            button1: "Account",
            href2: "logout",
            button2: "Log Out",
...
router.post('/', ensureAuthenticated, [
  ...
  db.query(sqlquery, (err, results, fields) => {
    ...
    if(!results[0]) {
      ...
      return res.redirect('/customerdeposite');
    }
    else {
      ...
      const depositeID = req.user.ID + req.body.AID;
      const sqlquery2 = "insert into deposits (ACC_ID, Cust_ID, Amount) values (" + req.body.AID + ", " + req.user.ID + ", " + req.body.Money + ");";
