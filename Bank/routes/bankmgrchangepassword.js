...
router.get('/', ensureAuthenticated, (req,res) => {
  const sqlquery = "select * from employee where ID = " + req.user.ID + " and employee.ID in (select Mgr from bank where Mgr = " + req.user.ID + ") ";
  db.query(sqlquery, (err, results, fields) => {
    if(err) return console.log(err);
    if(results[0]){
      return res.render('bankmgrchangepassword',{
        title: "Bank Manager Change Password",
        css: "profile",
        href1: "bankmgr",
        button1: "Account",
        href2: "logout",
...
router.post('/', ensureAuthenticated, [
  ...
  if(req.body.PW != req.body.CPW) {
    ...
    return res.redirect('/bankmgrchangepassword');
  }
  else {
